...
export const updateEvent = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if (updates.totalTickets < soldTickets.length) {
            throw new Error(
                `Không thể giảm số lượng vé dưới ${soldTickets.length} (số lượng vé đã bán)`
            );
        }
...
async function checkEventAvailability(ctx: any, eventId: any) {
    const event = await ctx.db.get(eventId);
    if (!event) throw new Error("Sự kiện không tồn tại");
    ...
    const activeOffers = await ctx.db
        ...
        .collect()
        .then(
            (entries: any[]) => entries.filter((e: any) => (e.offerExpiresAt ?? 0) > now).length
        );
...
export const joinWaitingList = mutation({
    ...
    handler: async (ctx, { eventId, userId }) => {
        ...
        if (existingEntry) {
            throw new Error("Đã có trong danh sách chờ của sự kiện này");
        }
        ...
        return {
            ...
            message: available
            ? "Vé đã được cung cấp - bạn có 30 phút để mua vé"
            : "Bạn đã được thêm vào danh sách chờ - bạn sẽ nhận được thông báo khi có vé khả dụng",
        };
    },
});
...
export const search = query({
    ...
    handler: async (ctx, { searchTerm }) => {
        ...
        // Hàm chuẩn hóa: xóa dấu và đưa về chữ thường
